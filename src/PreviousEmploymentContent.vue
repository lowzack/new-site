<template>
  <div class="p-4">
    <h1 class="mt-1 border-bottom border-primary">Previous Employment</h1>
    <div class="row justify-content-sm-center p-4">
      <div class="col-sm-9 d-flex align-items-center">
        <div class="d-block w-100r">
          <h2>Information Technology and Services</h2>
          <h3>Web Application Developer <small>August 2015 - July 2016</small></h3>
          <p class="description">Develop and maintian internal tools to streamline workflows of an IT support organization</p>
          <ul>
            <li>Build and maintain a web application for training of new technicians, allowing for additional metrics and insight into the training process</li>
            <li>Performed a major refactor of an aging ticketing and asset management application in order to decrease future maintainence costs</li>
          </ul>
        </div>
      </div>
      <div class="col-sm-3 text-center d-flex align-items-center">
        <img class="img-fluid mx-auto" src="https://www.rit.edu/its/system/files/logo-home_0.png"/>
      </div>
    </div>
    <div class="row justify-content-sm-center p-4">
      <div class="col-sm-9 d-flex align-items-center">
        <div class="d-block w-100r">
          <h2>1stdibs.com</h2>
          <h3>Frontend Engineer Intern <small>June 2015 - August 2015</small></h3>
          <p class="description">Develop and maintian internal tools to streamline workflows of an IT support organization</p>
          <ul>
            <li>Implemented complex user interactions using JavaScript and HTML.</li>
            <li>Leveraged best practices to create modularized reusable code.</li>
          </ul>
        </div>
      </div>
      <div class="col-sm-3 text-center d-flex align-items-center">
        <img class="img-fluid mx-auto" src="https://closetfullofcash.com/wp-content/uploads/1stdibs_logo.png"/>
      </div>
    </div>
  </div>
</template>

<script>
</script>

<style lang="scss" scoped>

</style>
